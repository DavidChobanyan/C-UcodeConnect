...
char *pathfinder(char *route, char *dist, t_island *islands, int *map, information data) {
    if (data.start == data.finish) {
        ...
        char *nr;
        nr = mx_strnew(mx_strlen(route) + mx_strlen(dist) + length + 42);
        mx_strcpy(nr, route);
        mx_strcat(nr, dist);
        mx_strcat(nr, sum);
        mx_strcat(nr, "\n========================================\n");
    ...
    char *or = mx_strdup(&(route[get_current_index(route)]));

    for (int i = 0; i < data.count; i++) {
        if (islands[data.start].length - islands[i].length == map[data.start * data.count + i]) {
            if(map[data.start * data.count + i] != -1){
                if (i==0) {
                    char *buf_PR = mx_strjoin(route, or);
                    mx_strdel(&route);
                    route = buf_PR;
                }
                char *new_route;
